...
test("codex desktop is local-only and not published by tap automation", () => {
  ...
  assert.match(localInstaller, /brew reinstall --cask "\$local_cask_token"/)
  ...
  assert.match(makefile, /CODEX_DESKTOP_CONVERSION_COMMIT \?= self-hosted/)
  ...
  assert.match(pipeline, /process\.env\.CODEX_DESKTOP_CONVERSION_COMMIT \|\| "self-hosted"/)
  assert.match(packageScript, /process\.env\.CODEX_DESKTOP_CONVERSION_COMMIT \|\| "self-hosted"/)
