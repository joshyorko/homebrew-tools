...
test("codex desktop official flow is normal and legacy DMG conversion stays explicit", () => {
  ...
  const setupAdapter = readFileSync(new URL("../../../scripts/setup-codex-desktop-local.sh", import.meta.url), "utf8")
  const featureWizard = readFileSync(new URL("../../../scripts/codex-desktop-feature-wizard.py", import.meta.url), "utf8")
  ...
  assert.match(pipeline, /tap\.file\("config\/codex-desktop-linux-features\.json"\)/)
  assert.match(pipeline, /enabled\.length === 0[^\n]+baseVersion[^\n]+features\./)
  ...
  assert.match(localInstaller, /CODEX_DESKTOP_BUNDLE_PARENT/)
  assert.match(localInstaller, /CODEX_DESKTOP_STOP_RUNNING/)
  assert.match(localInstaller, /CODEX_DESKTOP_ALLOW_RUNNING_INSTALL/)
  assert.match(localInstaller, /XDG_CACHE_HOME/)
  assert.match(localInstaller, /mktemp -d "\$bundle_parent\/codex-desktop-local\.XXXXXX"/)
  assert.match(localInstaller, /discover_codex_desktop_pids/)
  assert.match(localInstaller, /guard_running_codex_desktop/)
  assert.match(localInstaller, /Refusing to install Codex Desktop while bundle-backed processes are running/)
  assert.match(localInstaller, /Codex CLI\/MCP helpers using the installed Desktop bundle/)
  assert.match(localInstaller, /make codex-desktop-rebuild-relaunch/)
  assert.match(localInstaller, /CODEX_DESKTOP_STOP_RUNNING=1/)
  assert.match(localInstaller, /CODEX_DESKTOP_ALLOW_RUNNING_INSTALL=1/)
  ...
  assert.match(localInstaller, /--dmg-source/)
  assert.match(localInstaller, /--result-file/)
  assert.match(localInstaller, /Upstream DMG acceptance verdict/)
  assert.match(localInstaller, /Candidate was not installed/)
  assert.match(localInstaller, /--stop-running/)
  assert.match(localInstaller, /--allow-running/)
  ...
  assert.match(localInstaller, /HOMEBREW_DEVELOPER=1 brew untap "\$temp_tap_name"/)
  assert.doesNotMatch(localInstaller, /brew untap --force "\$temp_tap_name"/)
  ...
  assert.match(setupAdapter, /codex-desktop-feature-wizard\.py/)
  assert.match(setupAdapter, /--conversion-commit "\$resolved_commit"/)
  assert.match(setupAdapter, /--linux-features "\$feature_argument"/)
  assert.match(setupAdapter, /setup cancelled; no build started/)
  assert.match(featureWizard, /Gtk4|"Gtk", "4\.0"/)
  ...
  assert.doesNotMatch(pipeline, /depends_on cask: "codex"/)
  assert.match(pipeline, /curl -fsSL https:\/\/chatgpt\.com\/codex\/install\.sh \| sh/)
  assert.match(pipeline, /CODEX_DESKTOP_SKIP_CLI_INSTALL/)
