...
test("codex desktop release is driven by codex-desktop-linux dispatch", () => {
  ...
  assert.match(workflow, /auto-update-status[\s\S]*--codex-desktop-conversion-commit="\$\{\{ github\.event\.client_payload\.conversion_sha \}\}"/)
  assert.match(workflow, /release-bundle\s+--package-id="\$\{\{ matrix\.package_id \}\}"/)
  assert.match(workflow, /release-bundle[\s\S]*--codex-desktop-conversion-commit="\$\{\{ github\.event\.client_payload\.conversion_sha \}\}"/)
