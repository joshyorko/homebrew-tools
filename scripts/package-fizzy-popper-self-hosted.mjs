...
function findNpmPackTarball(packDir) {
  const matches = readdirSync(packDir)
    .filter((entry) => /^fizzy-popper-.*\.tgz$/.test(entry))
    .sort()

  if (matches.length !== 1) {
    throw new Error(`Expected exactly one fizzy-popper npm tarball in ${packDir}, found ${matches.length}`)
  }
...
function main() {
  ...
  if (!upstreamDirArg || !npmPackDirArg || !version || !outputPathArg) {
    throw new Error(
      "Usage: package-fizzy-popper-self-hosted.mjs --upstream-dir <dir> --npm-pack-dir <dir> --version <version> --output <tar.gz>",
    )
  }
  ...
  try {
    ...
    if (!existsSync(join(packageDir, "dist"))) {
      throw new Error(`Missing build output at ${join(packageDir, "dist")}`)
    }
