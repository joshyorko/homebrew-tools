...
function main() {
  ...
  if (!upstreamDirArg || !binaryArg || !version || !outputPathArg) {
    throw new Error(
      "Usage: package-eitype.mjs --upstream-dir <dir> --binary <path> --version <version> --output <tar.gz>",
    )
  }
  ...
  if (!existsSync(binaryPath)) {
    throw new Error(`Missing built eitype binary at ${binaryPath}`)
  }

  const stageRoot = mkdtempSync(join(tmpdir(), "eitype-homebrew-"))
  ...
  try {
    ...
    copyFileSync(binaryPath, join(libexecDir, "eitype"))
